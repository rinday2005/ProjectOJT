...
const AddMemberModal: React.FC<AddMemberModalProps> = ({ isOpen, onClose, onPatientAdded }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        relation: 'Patient',
        address: '',
        latitude: 10.762622, // Default HCMC coordinates
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setSubmitting(true);
            // Link resolving utility inside PatientRequest handles key naming automatically,
            // but let's send both formats to be absolutely robust
            const payload = {
                ...formData,
            ...
            await familyApi.createPatient(payload);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose} className="max-w-md">
            <DialogContent className="max-w-md bg-white rounded-[2rem] border-stone-100 p-8 font-['Public_Sans']">
                <DialogHeader>
                    <DialogTitle className="text-2xl font-black text-stone-900 tracking-tight">Add Family Member</DialogTitle>
                    <DialogDescription className="text-stone-400 text-xs">
                        Create a health profile record for a relative under your management.
                    </DialogDescription>
                </DialogHeader>
